...
from gaussian_renderer import render, half_render
import torchvision
...
def render_set_linear(model_path, name, iteration, views, gaussians, pipeline, background, args, refine=None):
    ...
    for idx, view in enumerate(tqdm(wide_views, desc="Rendering progress")):
        if idx in generate_idxs:
            ...
            c_views = c_views[N_C:]
            for ii in range(0, len(linear_views)-1):
                view = linear_views[ii]
                c = c_views[ii]
                
                #rendering = render(view, gaussians, pipeline, background, info={"c":c, "target":args.target})
                rendering = half_render(view, gaussians, pipeline, background, info={"c":c, "target":args.target})
                render_image = rendering["render"].clamp(0., 1.).unsqueeze(0)
                torchvision.utils.save_image(render_image, os.path.join(render_path, str(idx),  '0%d.png'%(ii)))
            #print(uw_views[idx].image_name)
            img_first = cv2.imread(args.source_path+'/images/'+uw_views[idx].image_name+'.jpg',flags=1)
            img_end = cv2.imread(args.source_path+'/images/'+wide_views[idx].image_name+'.jpg',flags=1)
            cv2.imwrite(os.path.join(render_path, str(idx),  '0%d.png'%(0)),img_first)
            cv2.imwrite(os.path.join(render_path, str(idx),  '0%d.png'%(31)),img_end)
