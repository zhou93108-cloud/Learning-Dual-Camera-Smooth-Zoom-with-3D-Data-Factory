...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, args, refine=None):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        if view.image_name.split('_')[0] == "w":
            ...
            rendering2 = half_render(view, gaussians, pipeline, background, info={"c":1.0, "target":args.target})
            torchvision.utils.save_image(rendering2["render"], os.path.join(render_path, 'half_' + view.image_name + '.png'))
            gt = view.original_image[0:3, :, :]
