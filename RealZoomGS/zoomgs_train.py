...
from gaussian_renderer import get_delta
...
def training(dataset, opt, pipe, args):
    ...
    if args.stage == "uw_pretrain":
        ...
        ema_loss_for_log = 0.0
        ema_loss5_for_log = 0.0
        ema_loss6_for_log = 0.0
        first_iter += 1
        ...
        for iteration in range(first_iter, opt.iterations + 1):
            ...
            deta_x_end, deta_c_end = render_pkg_wide["deta_x"], render_pkg_wide["deta_c"]
            ...
            '''
            # N_I = 5
            # c_views = np.concatenate((np.zeros(0), np.linspace(0., 1., N_I)), 0)
            # deta_x, deta_c = get_delta(gaussians, deta_x, deta_c, c_views, N_I, deta_x_end, deta_c_end)
            # for i in range(N_I-1):
            #     deta_x[i] = deta_x[i] - deta_x[i+1]
            #     deta_c[i] = deta_c[i] - deta_c[i+1]
            # for i in range(N_I-2):
            #     deta_x[i] = deta_x[i] - deta_x[i+1]
            #     deta_c[i] = deta_c[i] - deta_c[i+1]
            # loss5 = torch.abs(deta_x).mean() 
            # loss6 = torch.abs(deta_c).mean()
