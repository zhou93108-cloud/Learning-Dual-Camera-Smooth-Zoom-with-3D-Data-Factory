...
def interpolate(I0, I1, timestep):
    ...
    I1 = (torch.tensor(I1.transpose(2, 0, 1)).to(device) / 255.).unsqueeze(0)s
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_dir', type=str, default='./real_results_add1/', help='where to save image results')
    args = parse_args(parser)

    args.save_dir = os.path.join(args.log_dir, args.save_dir)
    
    model = Model(args)
    model.load_model(args.log_dir, suffix=99, rank=0)
    ...
    data_dir = os.path.join(args.dataset_dir, "syn2")
    ids = os.listdir(data_dir)
    for id in ids:
        psnr_list = []
        ssim_list = []
        lpips_list = []
        files = sorted(glob.glob(os.path.join(os.path.join(data_dir, id), '*.png')), key=lambda f:int(f.split('/')[-1].split('.')[0].split('_')[-1]))
