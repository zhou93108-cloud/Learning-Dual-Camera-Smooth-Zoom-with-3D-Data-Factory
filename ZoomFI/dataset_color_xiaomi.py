...
class DZDataset(Dataset):
    ...
    def __getitem__(self, index): 
        ...
        # 正态分布取值
        brightnesses = np.random.normal(1.0492,0.0548,100)
        contrasts = np.random.normal(0.9294,0.0511,100)
        saturations = np.random.normal(1.0168,0.0888,100)
        hues = np.random.normal(0.0021,0.0098,100)
        brightnesses = [m for m in brightnesses if 1.0492-2*0.0548 < m < 1.0492+2*0.0548]
        contrasts = [m for m in contrasts if 0.9294-2*0.0511 < m < 0.9294+2*0.0511]
        saturations = [m for m in saturations if 1.0168-2*0.0888 < m < 1.0168+2*0.0888]
        hues = [m for m in hues if 0.0021-2*0.0098 < m < 0.0021+2*0.0098]
