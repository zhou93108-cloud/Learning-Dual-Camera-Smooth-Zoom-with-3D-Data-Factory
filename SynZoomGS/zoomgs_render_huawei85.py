    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        if idx in ids:
            if view.T[2] > 0.6 or view.T[2] < -0.6:
                ...
                ys = np.random.normal(-0.5630,0.1803,100)
                zs = np.random.normal(0.2100,0.3648,100)
                xs = [m for m in xs if 0.0654-2*0.0829 < m < 0.0654+2*0.0829]
                ys = [m for m in ys if -0.5630-2*0.1803 < m < -0.5630+2*0.1803]
                zs = [m for m in zs if 0.2100-2*0.3648< m < 0.2100+2*0.3648]
                ...
                makedirs(os.path.join(render_path, str(idx)+'_h85'), exist_ok=True)
                ...
                for ii in range(0, len(linear_views)):
                    ...
                    torchvision.utils.save_image(rendering, os.path.join(render_path, str(idx)+'_h85',  '%04d.png'%(ii+1)))
