    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        if idx in ids:
            if view.T[2] > 0.6 or view.T[2] < -0.6:
                ...
                # huawei:
                xs = np.random.normal(0.0654,0.0829,100)
                ys = np.random.normal(-0.4630,0.1803,100)
                zs = np.random.normal(0.2100,0.3648,100)
                xs = [m for m in xs if 0.0654-1*0.0829 < m < 0.0654+1*0.0829]
                ys = [m for m in ys if -0.4630-1*0.1803 < m < -0.4630+1*0.1803]
                zs = [m for m in zs if 0.2100-1*0.3648< m < 0.2100+1*0.3648]
                ...
                # fovx_w = 0.65*math.tan(view.FoVx * 0.5)
                # fovy_w = 0.65*math.tan(view.FoVy * 0.5)
                fovx_w = 0.7*math.tan(view.FoVx * 0.5)
                fovy_w = 0.7*math.tan(view.FoVy * 0.5)
                w_FoVx = math.atan(fovx_w) * 2
                w_FoVy = math.atan(fovy_w) * 2
                view.image_width = 1216
                view.image_height = 1632

                w_view = Camera(colmap_id=view.colmap_id, R=view.R, T=view.T + [x,y,z] , FoVx=w_FoVx, FoVy=w_FoVy , image=view.image, gt_alpha_mask=None, 
                                image_name=view.image_name, uid=view.uid)
                
                linear_views = generate_linear_camera(WideCamera(view), WideCamera(w_view), N_C, N_I)

                makedirs(os.path.join(render_path, str(idx)+'_h'), exist_ok=True)
                ...
                for ii in range(0, len(linear_views)):
                    ...
                    torchvision.utils.save_image(rendering, os.path.join(render_path, str(idx)+'_h06',  '%04d.png'%(ii+1)))
